...
def gen_agent(name, has_separate_host_device_driver, root_dir, vendor,
              license_header="", module_type="", interface_name="", wires=[], gen_core_file=True, gen_bazel_file=True):
    # set sub name
    ...
    for tup in agent_srcs:
        ...
        if not gen_core_file and src_suffix == ".core":
            ...
            tpl = Template(filename=str(importlib_resources.files(
                'scripts').parent / "templates" / "fusesoc" / ftpl))
            ...
            tpl = Template(filename=str(importlib_resources.files(
                'scripts').parent / "templates" / "agent" / ftpl))
